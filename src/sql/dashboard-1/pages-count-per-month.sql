-- === Total Pages per Month, by version ===
--
SELECT year_and_month, count(page_id) as Pages, min(application_version_int) / 100 as version
FROM
(
  ...
  (
    ...
    (
      ...
      *
      ...
      WHERE event = 'USER_VISITED'
      AND timestamp >= date('2022-01-01')
      --LIMIT 50
    )
  )
  GROUP BY year_and_month, page_id, application_version_int
)
